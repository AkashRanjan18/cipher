...
/** Marks a run() result as a refusal: shown alone, never with an order summary. */
const FAILED = "\u0000";
...
export function Sana({
  ...
  async function run(spec: OrderSpec): Promise<string> {
    ...
    if (!entry) {
      ...
      if (held.qty <= 0) return `${FAILED}You have no ${market} to set a stop loss or target on.`;
      /*
      ...
      if (!ok) return `${FAILED}I couldn't save that order. Nothing was placed — try again.`;
      return armedLine(spec.exits.length, held.costBasis);
    }

    /* ── a resting limit order ── */
    if (entry.trigger) {
      if (!price) return `${FAILED}No live price right now, so nothing was placed. Try again in a moment.`;
      /*
    ...
    /* ── fill now ── */
    if (!price) return `${FAILED}No live price right now, so nothing was bought or sold.`;

    if (!mint) return `${FAILED}${market} can't be traded on cipher.`;
